...
namespace myOpenCV {
	...
	int saturationSettingGPU(int param, GpuMat& src, GpuMat& res) {
	...
	int saturationSettingLocal(int param, MatIterator_<Vec3b> it, MatIterator_<Vec3b> end) {
		float localParam = (float)param / 255;
		clock_t time;
		time = clock();
		for (it, end; it != end; ++it)
		...
		cout << "Thread Time" << ((float)clock() - time) / CLOCKS_PER_SEC << endl;

		return 0;
	}

	int saturationSettingPart(int param, MatIterator_<Vec3b> it, MatIterator_<Vec3b> end, int threadNumber, vector<thread> & threads) {
		...
		if (threadNumber > 2) {
			...
			saturationSettingPart(param, it, mid1, threadNumber / 2, threads);
			saturationSettingPart(param, mid2, end, (threadNumber % 2) + (threadNumber / 2), threads);
		}
		else {
			if (threadNumber == 1) {
				threads.push_back(thread(saturationSettingLocal, param, it, end));
			}
			if (threadNumber == 2) {
				threads.push_back(thread(saturationSettingLocal, param, it, mid1));
				threads.push_back(thread(saturationSettingLocal, param, mid2, end));
			}
		}
		return 0;
	}
	...
	int saturationSettingThreaded(Mat & src, Mat & res, int param) {
		...
		saturationSettingPart(param, it, end, n, threads);
	...
	static void on_trackbar(int, void*)
	{
		clock_t time;
		time = clock();
		saturationSettingThreaded(img, newImg, alpha_slider);
